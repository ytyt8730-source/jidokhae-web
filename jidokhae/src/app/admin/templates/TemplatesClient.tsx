      ...
      {templateCategories.map((category, idx) => (
        category.templates.length > 0 && (
          <div key={idx} className="card p-6">
            <h2 className="font-semibold text-warm-900 mb-4">{category.name}</h2>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <Bell size={16} className={template.is_active ? 'text-brand-500' : 'text-warm-400'} />
                        <span className="font-medium text-warm-900">{template.name}</span>
                        {!template.is_active && (
                          <span className="text-xs bg-warm-200 text-warm-600 px-2 py-0.5 rounded">
                            비활성
                          </span>
                        )}
                      </div>
                      {template.description && (
                        <p className="text-sm text-warm-500 mb-2">{template.description}</p>
                      )}
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <button
                        ...
                        disabled={isLoading}
                        className="p-2 transition-colors rounded-lg hover:bg-warm-100"
                        title={template.is_active ? '비활성화' : '활성화'}
                      >
                        {template.is_active ? (
                          <ToggleRight size={24} className="text-brand-500" />
                        ) : (
                          <ToggleLeft size={24} className="text-warm-400" />
                        )}
                      </button>
                    </div>
                  </div>
    ...
    >
      ...
      >
        <div className="sticky top-0 bg-white border-b border-warm-200 px-6 py-4 flex items-center justify-between">
          <h3 className="font-semibold text-warm-900">템플릿 수정</h3>
          <button
            onClick={onClose}
...
function PreviewModal({
  ...
  return (
    ...
    >
      ...
      >
        <div className="sticky top-0 bg-white border-b border-warm-200 px-6 py-4 flex items-center justify-between">
          <h3 className="font-semibold text-warm-900">미리보기</h3>
          <button
            onClick={onClose}
            className="p-1 hover:bg-warm-100 rounded-lg transition-colors"
          >
        ...
        <div className="p-6 space-y-4">
          <div>
            <div className="text-sm font-medium text-warm-700 mb-2">템플릿: {template.name}</div>
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <pre className="whitespace-pre-wrap text-sm text-warm-800 font-sans">
                {previewMessage}
              </pre>
            </div>
          </div>
          ...
          <div className="bg-warm-50 rounded-lg p-4">
            <div className="text-xs font-medium text-warm-500 mb-2">샘플 데이터</div>
            <div className="grid grid-cols-2 gap-2 text-xs">
              {template.variables?.map((v) => (
                <div key={v} className="flex justify-between">
                  <span className="text-warm-500">#{`{${v}}`}</span>
                  <span className="text-warm-700">{sampleData[v] || '-'}</span>
                </div>
              ))}
            </div>
          </div>
