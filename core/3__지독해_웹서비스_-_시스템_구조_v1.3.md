...
| **Dual Mode 테마 개인화** | 회원별 선호 테마 DB 저장 | 기기 간 동기화, 일관된 분위기 제공 |
...
| **Next.js + next/font** | 서버 사이드에서 3종 폰트(Outfit, Noto Sans/Serif)를 미리 최적화해 CLS 없는 매끄러운 로딩 |
...
| **분위기** | Atmosphere | 테마 전환, 커버 이미지 로딩 | users (preferred_theme), meetings (cover_image_url) |
...
### 5.5 테마 전환 흐름 (신규)
회원: 테마 스위처 토글 (Electric ↔ Warm)
│
▼
클라이언트: 즉시 반영
│
├── HTML data-theme 속성 변경 (CSS Variables 적용)
├── 로컬 스토리지 업데이트 (비회원/초기 로딩용)
└── 폰트 전환: Electric(Outfit) ↔ Warm(Noto Serif KR)
│
▼
서버 (로그인 상태인 경우):
│
└── users.preferred_theme 필드 업데이트
(다음 로그인 시 해당 테마로 자동 설정)

### 5.6 알림 발송 흐름 (No-Emoji 정책 적용)
스케줄러: 매일 실행
│
▼
세그먼트별 대상 조회
...
| 테이블 | 용도 |
|--------|------|
| users | 회원 정보, 역할, 정기모임 자격 상태, 신규 회원 여부, **선호 테마** |
...
CREATE TABLE users (
  ...
  preferred_theme TEXT DEFAULT 'electric',  -- [NEW] electric, warm
  preferred_theme_updated_at TIMESTAMP WITH TIME ZONE,  -- [NEW] 테마 변경 추적
...
| **테마 개인화** | users.preferred_theme으로 회원별 선호 테마 저장, 기기 간 동기화 |
...
| 마이페이지 (Profile) | 내 정보, 참여 통계, 배지, 책장, **테마 변경** |
...
| 역할 | 권한 범위 |
|------|----------|
| member (일반 회원) | 본인 데이터 조회/수정, 모임 조회, 신청/취소, 테마 변경 |
...
5. **테마 전환:** 로그인 → 테마 변경 → 로그아웃 → 재로그인 → 테마 유지 확인
...
| **Dual Mode 테마** | users.preferred_theme + CSS Variables | ✅ |
