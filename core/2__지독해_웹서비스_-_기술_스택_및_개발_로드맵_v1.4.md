...
**디자인 기준:** 디자인 시스템 v3.3 (Source of Truth)
...
| 폰트 | 용도 | 모드 |
|------|------|------|
| **Outfit** | Electric Mode 헤드라인 | Electric Mode 전용 |
| **Noto Sans KR** | 본문, UI 텍스트 | 공통 (모드 무관) |
| **Noto Serif KR** | Warm Mode 헤드라인, 책 제목 | Warm Mode 전용 |
...
export default function RootLayout({ children }) {
  return (
    <html className={`${outfit.variable} ${notoSansKr.variable} ${notoSerifKr.variable}`}>
      <body>{children}</body>
    </html>
  )
}
...
- 모바일 반응형 구현이 쉬움
- 브랜드 톤(Electric Mode + Warm Mode) 커스터마이징 용이

**테마 스위칭 전략:**
- **`darkMode: 'class'` 옵션 사용 금지**
- `data-theme` 속성과 CSS Variables 기반 테마 스위칭 채택
- 테마 변경 시 HTML root 요소에 `data-theme="electric"` 또는 `data-theme="warm"` 적용

**구현 코드:**
```javascript
// tailwind.config.js
module.exports = {
  // darkMode 옵션 사용 금지 - 명시적으로 제거
  content: ['./app/**/*.{js,ts,jsx,tsx}', './components/**/*.{js,ts,jsx,tsx}'],
  theme: {
    extend: {
      colors: {
        // Semantic Colors (CSS Variables 참조)
      ...
      fontFamily: {
        display: 'var(--font-display)',  // 헤드라인
        body: 'var(--font-body)',        // 본문
        serif: 'var(--font-noto-serif)', // Warm 전용
      },
      borderRadius: {
        DEFAULT: '12px',
        lg: '16px',
...
:root[data-theme="electric"] {
  ...
  --color-accent: #CCFF00;       /* Acid Lime - 배경 전용, 텍스트 금지 */
  --color-muted: #64748B;
  
  /* Typography */
  --font-display: var(--font-outfit), var(--font-noto-sans), sans-serif;
...
:root[data-theme="warm"] {
  ...
  --color-accent: #8B4513;       /* Burnt Orange */
  --color-muted: #6B7280;
  
  /* Typography */
  --font-display: var(--font-noto-serif), serif;
  --font-body: var(--font-noto-sans), sans-serif;
  
  /* Warm Mode 특수 효과: Noise Texture */
  --noise-opacity: 0.03;
}

/* Warm Mode Background Noise */
...
**커스터마이징 원칙:**
- `border-radius`, `colors` 등을 디자인 시스템 v3.3에 맞춰 수정
...
**목표:** 기본 인프라 세팅, 핵심 화면 확인, 테마 시스템 동작 검증
...
**구현 범위:**
- 테마 스위칭 기반 구조 (`data-theme` + CSS Variables)
- 3종 폰트 로딩 (Outfit, Noto Sans KR, Noto Serif KR)
- KongIcon 커스텀 컴포넌트
...
**검증 포인트:**
- [ ] Electric Mode / Warm Mode 전환이 의도대로 동작하는가?
- [ ] 폰트가 모드별로 올바르게 적용되는가? (Electric: Sans, Warm: Serif 헤드라인)
...
**구현 범위:**
- 카카오 로그인
- 모임 신청 + 결제 (카카오페이 or 토스) - **Bottom Sheet 내 Sticky CTA**
- 취소 + 자동 환불 (환불 규정 적용)
- 취소 사유 입력
- 대기 신청 (결제 없이 대기만)
...
- [ ] 마이페이지 내 `<Trophy />` Glow가 테마별로 다르게 적용되는가?
...
| Phase | 내용 | 기간 | 누적 |
|-------|------|------|------|
| 0 | 기반 구축 + 테마 시스템 | 1~2주 | 2주 |
| 1 | 핵심 흐름 (결제/환불) + 3-Click | 2~3주 | 5주 |
| 2 | 알림톡 연동 | 1~2주 | 7주 |
...
2. **테마 스위칭:** `darkMode` 옵션 사용 금지. `data-theme` 속성과 CSS Variables 기반으로 구현.
3. **폰트 전략:** `next/font`를 통해 Outfit, Noto Sans KR, Noto Serif KR 3종 로딩. 모드별 헤드라인 폰트 차별화.
4. **색상 가독성:** Electric Mode에서 라임색(#CCFF00)은 배경으로만 사용, 텍스트로 사용 금지.
