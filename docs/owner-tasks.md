...
| # | 템플릿 코드 | 용도 | 우선순위 | 상태 |
|:-:|-------------|------|:--------:|:----:|
| 1 | `TRANSFER_DEADLINE_WARNING` | 입금 기한 임박 (6시간 전) | **높음** | [ ] |
| 2 | `WAITLIST_EXPIRED` | 대기 응답 기한 만료 | **높음** | [ ] |
| 3 | `REMINDER_3D` | 모임 3일 전 리마인드 | 중간 | [ ] |
| 4 | `MONTHLY_ENCOURAGE` | 월말 참여 독려 | 낮음 | [ ] |
| 5 | `ELIGIBILITY_WARNING` | 자격 만료 임박 (6개월 룰) | 낮음 | [ ] |
| 6 | `DORMANT_RISK` | 휴면 위험 알림 | 낮음 | [ ] |
| 7 | `ONBOARDING_RISK` | 온보딩 이탈 위험 | 낮음 | [ ] |
| 8 | `ADMIN_NOTICE` | 관리자 수동 공지 | 낮음 | [ ] |

> **참고:** 1~2번은 현재 Cron이 실행 중이나 템플릿이 없어 발송 실패합니다.
...
| 변수명 | 설명 | 예시 |
|--------|------|------|
| `#{메시지}` | 관리자가 입력한 공지 내용 | 2월 모임 일정이 변경되었습니다. |
