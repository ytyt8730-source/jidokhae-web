...
> **코드 참고:** `onboarding-first-meeting/route.ts` - `이름`, `모임명`, `다음모임일시`, `신청인원`, `책제목` 전송
...
> **코드 참고:** `reminder.ts` - `날짜`와 `시간`을 별도 변수로 전송합니다 (통합 `일시` 아님).
...
현재 자격이 곧 만료될 예정이오니 모임 일정을 확인해주세요.
```

**버튼:** 모임 일정 확인 → `https://jidokhae.com`

> **코드 참고:** `segment-notification.ts` - 현재 `이름` 변수만 전송합니다.
...
**본문:**
```
#{이름}님, 가입 후 아직 모임 참여 내역이 없어 안내드립니다.
...
> **참고:** 코드에서 `이름` 변수만 전달하므로 모임명은 사용하지 않습니다.
...
*최종 업데이트: 2026-02-11 (코드-템플릿 변수 교차 검증 완료, 6건 불일치 수정)*